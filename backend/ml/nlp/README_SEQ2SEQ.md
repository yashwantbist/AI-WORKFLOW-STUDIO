...
Create the branch from your completed Day 11 branch:

```powershell
git switch day11/causal-masking
git switch -c feature/encoder-decoder-transformer
```

After tests and visualization succeed:

```powershell
git add backend/ml/nlp/cross_attention.py backend/ml/nlp/seq2seq_transformer.py backend/ml/nlp/visualize_cross_attention.py backend/ml/nlp/README_SEQ2SEQ.md backend/tests/test_seq2seq_pipeline.py images/cross_attention.png
git commit -m "feat(ml): implement encoder-decoder transformer with cross-attention"
git push -u origin HEAD
```

## `.gitignore` and secret check
...
Look for accidentally tracked sensitive or generated files:

```powershell
git ls-files | Select-String -Pattern '(^|/)\.env$|\.pem$|\.key$|\.pt$|\.pth$|__pycache__'
```

The command should not list environment files, private keys, model
checkpoints, or Python cache files. The attention screenshot is safe and
should be committed as portfolio evidence.

## Definition of done

- [x] Encoder output interface implemented.
- [x] Multi-head cross-attention implemented.
- [x] Decoder consumes encoder representations.
- [x] Encoder, decoder, and cross-attention maps exposed.
- [x] Toy training and greedy inference implemented.
- [x] Three-panel visualization implemented.
- [x] Behavioral tests written.
- [ ] Run tests successfully in your `.venv`.
- [ ] Run toy inference and record measured output.
